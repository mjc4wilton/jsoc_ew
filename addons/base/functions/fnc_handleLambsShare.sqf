...
if (EGVAR(base,hasACRE)) then {
    ...
    private _jammers = missionNamespace getVariable [QEGVAR(acre,jammers), []];
    {
        ...
        if (_frequencyTx == GVAR(enemyFrequency)) then {
            ...
            private _distanceJammer = _obj distance2D _target;    // Uses distance2D as attenuation through air to an aircraft is minimal.
            private _distanceUnits = _unit distance2D _target;
            if ((_distanceJammer / 2) <= _distanceUnits) exitWith { // Divided by 2 to account for having too much static to communicate effectively.
                _return = true;
            };
        };
    } forEach _jammers;
